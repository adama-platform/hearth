...
> **1737 tests**, and most of them are not unit tests: they boot the whole server on a port and
...
> is only a backstop. Certificates over HTTP-01 answered by the same process. 1737 tests, most of
...
| 1737 tests | the suite; `just validate` checks this number against the surefire reports | kept true by the build |
